...
public:
    ...
    void removeFromList(Metadata* m) {
        ...
        if (m->prev == nullptr && m->next == nullptr) { // last one case
            ListHead = nullptr;
            std::cout << "Removing LAST1" << std::endl;
            return;
        }
        if (m->prev != nullptr) {
            m->prev->next = m->next;
...
public:
    ...
    void* allocate(size_t size) {
        if (size >= 128 * 1024) { // For large allocations
            ...
            std::cout << "order needed= " << order << std::endl;
    ...
    void free(void* ptr) {
        ...
        std::cout << "freeing size = " << block->size << std::endl;
        if (block->size >= 128 * 1024) {
            ...
            std::cout << "success unmap" << std::endl;
            big_counter--;
            big_bytes = big_bytes - size;
            std::cout << "success unmap3" << std::endl;
            return;
        }
        else {
            block->is_free = true;
            std::cout << "removing allocated size = " << block->size << std::endl;
    ...
    void mergeWithBuddy(Metadata* block) {
        ...
        if (block->buddy != nullptr) {
            ...
            std::cout << "buddy_size = " << buddy->size << "Free== " << buddy->is_free << std::endl;
            if (buddy &&
                ...
                */
                ...
                std::cout << "Adding to List order=" << block->order << std::endl;
...
void* srealloc(void* oldp, size_t size) {
    ...
    if (new_block != NULL) {
        ...
        memcpy(new_block, oldp, bytes_to_copy); // Copy data
        free(oldp); // Free the old block
    }
...
size_t _num_allocated_blocks() {
    ...
    for (int i = 0; i <= int(MAX_ORDER); i++) {
        Metadata* curr = buddy_alc.free_lists[i].ListHead;
        while (curr != nullptr) {
            std::cout << "One Size: " << curr->size << std::endl;
    ...
    std::cout << "free+alloc = " << allocated_blocks << std::endl;
    std::cout << "free+alloc+big = " << allocated_blocks+buddy_alc.big_counter << std::endl;
